...
* **基因序列定义 (Genetic Sequence Definition)**: AGP提供了一个由五个逻辑依赖层构成的档案，将模糊的概念（如“智能”）分解为25个具体的、可量化的核心属性，并最终形成一个清晰的数字“基因序列”。
...
这是AGP最核心的表达方式。我们将一个Agent的25个核心属性值，按照五层依赖结构的顺序进行拼接，并用 `-` 分隔，形成一个唯一的**基因序列**。
...
一个Agent的定义档案(ADP)是一个由25个属性构成的特征向量，分为五个**依赖层 (Layers)**。
